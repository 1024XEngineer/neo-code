...
4. **开启应用权限（必须这 3 项）** — 「开发配置 → 权限管理 → 应用身份权限」中确保已开通：
   ...
   - 对以上每一项，点击右侧「相关 API/事件」，确认已经关联并启用对应的机器人消息事件
...
|------|--------|------|
| Windows | `\\.\pipe\neocode-gateway` | 命名管道 |
| macOS / Linux | `$HOME/.neocode/run/gateway.sock` | Unix 域套接字（IPC） |
...
```bash
# 开发模式 (go run)
go run ./cmd/neocode-gateway --listen "$HOME/.neocode/run/gateway.sock" --http-listen "127.0.0.1:18181" --workdir "/home/you/project"

# 安装模式 (neocode)
neocode gateway --listen "$HOME/.neocode/run/gateway.sock" --http-listen "127.0.0.1:18181" --workdir "/home/you/project"
```
...
| 参数 | 必填 | 默认值 | 说明 |
|------|:---:|--------|------|
| `--listen` | 是* | — | IPC 监听地址。Windows 用命名管道 `\\.\pipe\<name>`；macOS/Linux 用 Unix socket（如 `$HOME/.neocode/run/gateway.sock`） |
...
```bash
# 开发模式 (go run)
go run ./cmd/neocode adapter feishu --ingress sdk --gateway-listen "$HOME/.neocode/run/gateway.sock"

# 安装模式 (neocode)
neocode adapter feishu --ingress sdk --gateway-listen "$HOME/.neocode/run/gateway.sock"
```
...
```bash
# 开发模式 (go run)
go run ./cmd/neocode-gateway --listen "$HOME/.neocode/run/gateway.sock" --http-listen "127.0.0.1:18181" --workdir "/path/to/project"

# 安装模式 (neocode)
neocode gateway --listen "$HOME/.neocode/run/gateway.sock" --http-listen "127.0.0.1:18181" --workdir "/path/to/project"
```
...
```bash
# 开发模式 (go run)
go run ./cmd/neocode adapter feishu --ingress webhook --gateway-listen "$HOME/.neocode/run/gateway.sock" --listen "127.0.0.1:18080"

# 安装模式 (neocode)
neocode adapter feishu --ingress webhook --gateway-listen "$HOME/.neocode/run/gateway.sock" --listen "127.0.0.1:18080"
```
...
1. Gateway 是否在运行
2. Adapter 的 `--gateway-listen` 是否与 Gateway 的 `--listen` **完全一致**（Windows 检查 `\\.\pipe\` 前缀；macOS/Linux 检查 Unix socket 路径）
