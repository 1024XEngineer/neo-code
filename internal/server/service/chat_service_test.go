...
func (p *captureChatProvider) Chat(_ context.Context, messages []domain.Message) (<-chan string, error) {
...
func TestSendInjectsToolSchemaIntoSystemContext(t *testing.T) {
	...
	gateway := NewChatService(mem, work, todo, role, provider, "[TOOL_SCHEMAS]\n{}")
...
func TestSendPrependsRolePromptWhenSystemMessageExistsLater(t *testing.T) {
	...
	gateway := NewChatService(mem, nil, &stubTodoService{}, role, provider, "[TOOL_SCHEMAS]\n{}")
	out, err := gateway.Send(context.Background(), &domain.ChatRequest{
		Messages: []domain.Message{
	...
	if !strings.Contains(provider.messages[0].Content, "[TOOL_SCHEMAS]") {
		t.Fatalf("expected tool schema to be injected, got %q", provider.messages[0].Content)
	}
}
...
func TestInjectSystemContextPreservesExistingLeadingSystemContent(t *testing.T) {
	...
	got := injectSystemContext(messages, "role prompt", "[TOOL_SCHEMAS]\n{}")
	if len(got) != 2 {
	...
	if !strings.Contains(got[0].Content, "[TOOL_SCHEMAS]") {
		t.Fatalf("expected tool schema in leading system message, got %q", got[0].Content)
	}
