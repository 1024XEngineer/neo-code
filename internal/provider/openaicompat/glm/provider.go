...
func (p *Provider) generateOnce(ctx context.Context, payload chatcompletions.Request, tc *providertypes.ThinkingConfig, events chan<- providertypes.StreamEvent) error {
	...
	if tc != nil {
		body, err = injectGLMParams(body, *tc)
		if err != nil {
...
// injectGLMParams 注入 GLM 特有的 enable_thinking 和 chat_template_kwargs 参数。
func injectGLMParams(body []byte, tc providertypes.ThinkingConfig) ([]byte, error) {
	var raw map[string]any
	if err := json.Unmarshal(body, &raw); err != nil {
		return nil, err
	}

	raw["enable_thinking"] = tc.Enabled
	raw["chat_template_kwargs"] = map[string]any{
		"enable_thinking": tc.Enabled,
		"clear_thinking":  !tc.Enabled,
	}
