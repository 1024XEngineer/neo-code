...
为解决多数文档平台无法直接点击 `neocode://` 的限制，引入本机 HTTP daemon 入口并与 URL Scheme 并存：

- 入口：`http://neocode:18921/run?...`、`http://neocode:18921/review?...`
- `daemon` 不再执行 `HTTP -> neocode:// -> ParseNeoCodeURL` 双序列化；而是直接将 HTTP 参数映射为 `protocol.WakeIntent`，复用与 `url-dispatch` 相同的 IPC/RPC 派发核心。
...
2. 验收通过后，再在后续 PR 中移除 `neocode://` 系统注册链路。
