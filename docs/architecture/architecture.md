...
理解 NeoCode 架构的关键不是记住它有哪些模块，而是理解它**面对各种约束和权衡时，为什么做出了这些选择而不是那些**。以下五个设计张力是后续所有模块和流程设计的根源。
...
**存在理由：** D4（工具执行可控）和 D5（Human-in-the-Loop）的工程载体。Tools 层是系统中唯一被允许执行文件 I/O、Bash 命令、网络请求和 MCP 外部工具的模块。任何模型可调用的能力必须在此注册——这是"能力入口收敛"原则（§5.4 原则 2）的实现。
